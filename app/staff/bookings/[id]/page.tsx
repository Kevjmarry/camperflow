...
export default function BookingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchBooking = async () => {
    try {
      ...
      if (data) {
        ...
        fetchChecklistTemplates(data.company_id);
  ...
  const fetchRedactedBooking = async () => {
    try {
      ...
      fetchChecklistTemplates(bookingData.company_id);
      fetchChecklistInstances();
    } catch (err: any) {
  ...
  const fetchChecklistTemplates = async (companyId: string) => {
    try {
      const { data, error } = await supabase
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (formData.vehicle_id && isActiveStatus(formData.status)) {
      ...
      if (!isAvailable) {
        setError("Cannot save: Vehicle is not available for the selected dates");
        setSaving(false);
        return;
      }
    }
    ...
    try {
      ...
      if (!updateData || updateData.length === 0) {
        setError('Permission denied: You do not have access to update this booking');
        setSaving(false);
        return;
      }
  ...
  const handleCancelBooking = async () => {
    if (!confirm('Cancel this booking? This will set status to cancelled.')) return;
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!deleteData || deleteData.length === 0) {
        setError('Permission denied: You do not have access to delete this booking');
        setSaving(false);
        return;
      }
  ...
  const getVehicleName = () => {
    if (canManage) {
      if (!formData.vehicle_id) return 'Unassigned';
      const vehicle = vehicles.find(v => v.id === formData.vehicle_id);
      return vehicle ? `${vehicle.name} (${vehicle.registration_plate})` : 'Unassigned';
    } else {
      if (!vehicleInfo) return 'Unassigned';
    ...
    return (
      <PageContainer maxWidth="800px">
        <div className="surface" style={{ padding: 'var(--space-8)', textAlign: 'center' }}>
          ...
          <Link href="/staff/bookings" className="btn btn-primary">
            Back to bookings
          </Link>
        </div>
      </PageContainer>
  ...
  if (loading) {
    return (
      <PageContainer maxWidth="800px">
        <div className="surface" style={{ padding: 'var(--space-8)', textAlign: 'center' }}>
          <p style={{ color: 'rgb(var(--muted))' }}>Loading booking...</p>
        </div>
      </PageContainer>
    );
  }
    ...
    return (
      <PageContainer maxWidth="800px">
        <div className="surface" style={{ padding: 'var(--space-8)' }}>
          ...
          <Link href="/staff/bookings" className="btn btn-secondary">
            Back to bookings
          </Link>
        </div>
      </PageContainer>
              ...
              <div style={{ 
                ...
                <div>
                  ...
                  <div style={{ fontSize: '15px', fontWeight: 500, color: vehicleInfo ? 'rgb(var(--text))' : 'rgb(var(--muted))' }}>
                    {vehicleInfo ? `${vehicleInfo.name} (${vehicleInfo.registration_plate})` : 'Unassigned'}
                  </div>
                </div>
                <div>
                  <div style={{ fontSize: '13px', color: 'rgb(var(--muted))', marginBottom: 'var(--space-1)' }}>
            ...
            <div>
              ...
              {checklistInstances.length === 0 ? (
                <div style={{ 
                  ...
                  No checklists for this booking
                </div>
              ) : (
                    ...
                    >
                      <div style={{ flex: 1 }}>
                        <div style={{ 
                          ...
                          {instance.checklist_type} Checklist
                        </div>
                        <div style={{ fontSize: '13px', color: 'rgb(var(--muted))', textTransform: 'capitalize' }}>
                          {instance.status.replace('_', ' ')}
                        </div>
                      </div>
                      <Link
                        href={`/staff/checklists/${instance.id}`}
                      ...
                      >
                        {instance.status === 'completed' ? 'View checklist' : 'Open checklist'}
                      </Link>
                    </div>
                  ))}
                </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
          <div>
            ...
            <div style={{ 
              ...
              <div>Customer: <span style={{ color: 'rgb(var(--text))' }}>{booking.customer_name || '-'}</span></div>
              <div>Vehicle: <span style={{ color: 'rgb(var(--text))' }}>{getVehicleName()}</span></div>
              <div>Pickup: <span style={{ color: 'rgb(var(--text))' }}>{formatDate(booking.pickup_at)}</span></div>
              <div>Return: <span style={{ color: 'rgb(var(--text))' }}>{formatDate(booking.return_at)}</span></div>
            </div>
          </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: 'var(--space-4)' }}>
                <div>
                  <label htmlFor="customer_name" className="label">
                    Customer name {formData.status !== 'blocked' && <span style={{ color: 'rgb(var(--error))' }}>*</span>}
                  </label>
                  <input
                    id="customer_name"
                    name="customer_name"
                ...
                <div>
                  <label htmlFor="customer_phone" className="label">
                    Phone number {formData.status !== 'blocked' && <span style={{ color: 'rgb(var(--error))' }}>*</span>}
                  </label>
                  <input
                    id="customer_phone"
                    name="customer_phone"
