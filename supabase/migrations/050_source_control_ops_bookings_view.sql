...
SELECT b.id,
    ...
    v.operational_hold,
        CASE
            ...
            WHEN now() >= b.return_at THEN 'start_return'::text
    ...
    EXTRACT(epoch FROM b.pickup_at - now()) / 3600::numeric AS hours_to_pickup,
        CASE
            WHEN date(b.pickup_at) = CURRENT_DATE THEN 'pickup_today'::text
            WHEN date(b.return_at) = CURRENT_DATE THEN 'return_today'::text
            WHEN now() > b.return_at AND b.status <> 'completed'::booking_status THEN 'overdue_return'::text
            ELSE NULL::text
        END AS ops_flag,
        CASE
            WHEN now() > b.return_at AND b.status <> 'completed'::booking_status THEN true
            ELSE false
        END AS is_overdue,
        CASE
